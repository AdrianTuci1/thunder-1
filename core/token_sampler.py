...
class ThunderTokenSampler:
    ...
    def sample(self, latent_vectors, lm_head):
        ...
        latent_vectors: [B, L, D]
        """
        # Project Latent Space -> Vocabulary Logits
        logits = lm_head(latent_vectors) # [B, L, V]
        
        # We handle sampling token-by-token or sequence-by-sequence
        B, L, V = logits.shape
        all_sampled = []
        ...
        for b in range(B):
            batch_tokens = []
            for l in range(L):
                token_logits = logits[b, l, :]
                sampled_token = self._sample_logits(token_logits)
                batch_tokens.append(sampled_token)
            all_sampled.append(torch.cat(batch_tokens))
            
        return torch.stack(all_sampled)

    def _sample_logits(self, logits):
        """Internal method performing the actual temperature/Top-K/Top-P scaling on 1D logits [V]."""
